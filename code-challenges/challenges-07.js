...
//  + obj.firstName.slice(1)
   ...
   let upper = `my name is ${obj.firstName.charAt(0).toUpperCase() + obj.firstName.slice(1)} ${obj.lastName.charAt(0).toUpperCase() + obj.lastName.slice(1)} I am ${obj.age} YO, and I love ${obj.hobby}.`;
return upper;
...
const cv = arr[i];
if (cv.lastName === null && cv.yearsOfExperience > 1) {
    newArr.push({ fullName: `${cv.firstName}`, tech: `${cv.tech}`});
} 
else if(cv.yearsOfExperience >1) {
    newArr.push({ fullName: `${cv.firstName} ${cv.lastName}`, tech: `${cv.tech}` });
} else continue;
}
return newArr;
